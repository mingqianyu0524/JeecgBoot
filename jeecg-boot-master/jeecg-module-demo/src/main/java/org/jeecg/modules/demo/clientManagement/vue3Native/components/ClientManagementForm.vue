<template>
  <a-spin :spinning="loading">
    <a-form v-bind="formItemLayout">
      <a-row>
        <a-col :span="24">
          <a-form-item label="客户编号" v-bind="validateInfos.clientIndex">
	          <a-input-number v-model:value="formData.clientIndex" placeholder="请输入客户编号" style="width: 100%" disabled/>
          </a-form-item>
        </a-col>
        <a-col :span="24">
        ...
        <a-col :span="24">
          <a-form-item label="联系人" v-bind="validateInfos.contact">
	          <j-search-select v-model:value="formData.contact" dict="client_contact,name,name" :disabled="disabled" />
          </a-form-item>
        </a-col>
        <a-col :span="24">
    ...
    <a-tabs v-model:activeKey="activeKey" animated>
      <a-tab-pane tab="客户联系人" key="clientContact" :forceRender="true">
        <j-vxe-table
          :keep-source="true"
          resizable
          ref="clientContactTableRef"
...
<script lang="ts">
  ...
  import { queryClientContactListByMainId, queryDataById, saveOrUpdate } from '../ClientManagement.api';
  import { JVxeTable } from '/@/components/jeecg/JVxeTable';
  import {clientContactColumns} from '../ClientManagement.data';
  ...
  export default defineComponent({
    ...
    setup(props, {emit}) {
      ...
      const clientContactTableRef = ref();
      const clientContactTable = reactive<Record<string, any>>({
        loading: false,
        columns: clientContactColumns,
        dataSource: []
      });
      const activeKey = ref('clientContact');
      ...
      async function edit(row) {
        ...
        //子表数据
        const clientContactDataList = await queryClientContactListByMainId(row['id']);
        clientContactTable.dataSource = [...clientContactDataList];
      }
      ...
      const {getSubFormAndTableData, transformData} = useValidateAntFormAndTable(activeKey, {
        'clientContact': clientContactTableRef,
      });
