...
<script lang="ts" setup>
    ...
    import {formSchema,supplierContactColumns} from '../SupplierManagement.data';
    import {saveOrUpdate,supplierContactList} from '../SupplierManagement.api';
    import { VALIDATE_FAILED } from '/@/utils/common/vxeUtils'
    // Emits声明
    const emit = defineEmits(['register','success']);
    const isUpdate = ref(true);
    const formDisabled = ref(false);
    const refKeys = ref(['supplierContact', ]);
    const activeKey = ref('supplierContact');
    const supplierContact = ref();
    const tableRefs = {supplierContact, };
    ...
    async function reset(){
      await resetFields();
      activeKey.value = 'supplierContact';
