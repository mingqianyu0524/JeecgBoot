...
//		}
    ...
    public ModelAndView exportXls(SysDepart sysDepart,HttpServletRequest request) {
        ...
        LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
		ExportParams params = new ExportParams("部门列表数据", "导出人:" + user.getRealname(), "导出信息");
		params.setType(ExcelType.XSSF);
		params.setExclusions(new String[]{"英文名","排序","描述","机构类别","手机号","传真","地址","备注"});
	...
	public ModelAndView downTemplate() {
		...
		ModelAndView mv = new ModelAndView(new JeecgEntityExcelView());
		mv.addObject(NormalExcelConstants.FILE_NAME, "部门管理列表");
		mv.addObject(NormalExcelConstants.CLASS, SysDepart.class);
		ExportParams exportParams = new ExportParams("部门列表数据", "导入人:" + sysUser.getRealname(), "部门管理");
		exportParams.setType(ExcelType.XSSF);
		exportParams.setExclusions(new String[]{"英文名","排序","描述","机构类别","手机号","传真","地址","备注"});
		mv.addObject(NormalExcelConstants.PARAMS, exportParams);
		mv.addObject(NormalExcelConstants.DATA_LIST, new ArrayList<SysDepart>());
		return mv;
	}
